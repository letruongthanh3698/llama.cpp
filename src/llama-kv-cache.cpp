...
void llama_kv_cache::state_read(llama_io_read_i & io, llama_seq_id seq_id, llama_state_seq_flags flags) {
    ...
    for (uint32_t s = 0; s < n_stream_read; ++s) {
        ...
        // Upstream 298219f98 wrapped the data read in try/catch so a malformed blob fails the restore
        // instead of escaping as an exception. Both branches go inside it — the tagged path reads the
